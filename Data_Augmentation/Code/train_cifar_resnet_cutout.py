...
co_config_path ='configuration/cutout_config.yaml'
model_args = getYaml(model_config_path) 
aug_args = getYaml(co_config_path)

#저장할 루트 디렉토리   
CHECKPOINT_ROOT = "./experiment_results"
SAVE_DIR = os.path.join(CHECKPOINT_ROOT, f"{model_args['arch']}{model_args['depth']}_with_Cutout")
...
def main():
    ...
    transform_train = transforms.Compose([
        ...
        transforms.Cutout(probability = aug_args['p'],
                                 n_holes = aug_args['holes'], 
                                 length = aug_args['length'], 
                                 )
    ])
