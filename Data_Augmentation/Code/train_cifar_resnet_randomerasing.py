...
re_config_path ='configuration/re_config.yaml'
model_args = getYaml(model_config_path) 
aug_args = getYaml(re_config_path)

#저장할 루트 디렉토리   
CHECKPOINT_ROOT = "./experiment_results"
SAVE_DIR = os.path.join(CHECKPOINT_ROOT, f"{model_args['arch']}{model_args['depth']}_with_RandomErasing")
...
def main():
    ...
    transform_train = transforms.Compose([
        ...
        transforms.RandomErasing(probability = aug_args['p'],
                                 sl = aug_args['sl'], 
                                 sh = aug_args['sh'], 
                                 r1 = aug_args['r1'], 
                                 value= aug_args['value'])
    ])
